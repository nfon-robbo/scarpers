...
function formatSparkValue(label: string, v: number): string {
  ...
  if (label === "Body Battery") return `${Math.round(v)}%`;
  return `${Math.round(v)}`;
}
    ...
    Promise.all([
      ...
      const series: Record<string, SparkPoint[]> = {
        ...
        "Body Battery": toPoints((d) => {
          ...
          // accelerating drain: 2 / 3 / 4 / 5 pts per hour
          let passive = 0;
          if (hoursAwake <= 4) passive = hoursAwake * 2;
          else if (hoursAwake <= 8) passive = 8 + (hoursAwake - 4) * 3;
          else if (hoursAwake <= 12) passive = 20 + (hoursAwake - 8) * 4;
          else passive = 36 + (hoursAwake - 12) * 5;
          const active = load != null ? load * 0.05 : 0;
          // estimate starting battery from sleep score (rough, no HRV history here)
          const ss = mByDate.get(d)?.sleep_score;
          const start = ss != null ? Math.round(30 + (ss / 100) * 60) : 75;
          if (!isToday && load == null && ss == null) return null;
          return Math.max(0, Math.min(100, Math.round(start - passive - active)));
        }),
        ...
        <CardContent className="p-5 relative z-10">
          ...
          {(() => {
            ...
            <div className="flex-1 min-w-0 rounded-xl bg-[#111a2e] border border-border/30 divide-y divide-border/30">
              ...
              {(() => {
                // ── Recovery Focus — top 1-2 marching orders ──
                const ORDERS: Record<string, string> = {
                  "Deep Sleep": "Sleep 8+ hours tonight",
                  "Sleep Quality": "Sleep 8+ hours tonight",
                  "Sleep Debt": "Sleep 8+ hours tonight",
                  HRV: "Reduce stress & training volume",
                  "Resting HR": "Monitor for illness, prioritise rest",
                  "Yesterday's Load": "Easy training only today",
                  "Today's Effort": "Refuel & wind down early",
                  "Body Battery": "Active recovery only",
                };

                const order: Record<string, number> = { poor: 0, warning: 1, good: 2 };
                const issues = displayResult.factors
                  .filter((f) => f.status !== "good" && ORDERS[f.label])
                  .sort((a, b) => (order[a.status] ?? 9) - (order[b.status] ?? 9));

                // De-dupe identical orders (e.g. multiple sleep factors → one "Sleep 8+ hours")
                const seen = new Set<string>();
                const picks = issues
                  .filter((f) => {
                    const o = ORDERS[f.label];
                ...
                if (picks.length === 0 || suppressScore) return null;
