...
import SleepHealthMetrics from "@/components/insights/SleepHealthMetrics";
    ...
    <div className="space-y-4">
      ...
      {summary && (
        <div className="grid gap-3 grid-cols-2 lg:grid-cols-3">
          <KPICard icon={Footprints} label="Steps" value={summary.todaySteps != null ? summary.todaySteps.toLocaleString() : "—"} sub={summary.avgSteps7d != null ? `7d avg: ${summary.avgSteps7d.toLocaleString()}` : "No data"} color="text-primary" />
          <KPICard icon={Heart} label="Resting HR" value={summary.todayRHR != null ? `${Math.round(summary.todayRHR)} bpm` : "—"} sub={summary.avgRHR7d != null ? `7d avg: ${summary.avgRHR7d} bpm` : "No data"} color="text-destructive" />
        </div>
      )}
      ...
      {metrics.some(m => m.steps != null) && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2"><Footprints className="w-4 h-4 text-primary" />Daily Steps</CardTitle>
            <CardDescription>Step count over time</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={metrics.filter(m => m.steps != null).map(m => ({ date: fmtDate(m.date), steps: m.steps }))}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                <XAxis dataKey="date" tick={{ fontSize: 10 }} className="fill-muted-foreground" interval="preserveStartEnd" />
                <YAxis tick={{ fontSize: 11 }} className="fill-muted-foreground" />
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} labelStyle={{ color: "hsl(var(--foreground))" }} formatter={(v: number) => [v.toLocaleString(), "Steps"]} />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {metrics.some(m => m.resting_heart_rate != null) && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2"><Heart className="w-4 h-4 text-destructive" />Resting Heart Rate</CardTitle>
              <CardDescription>Lower is generally better for endurance athletes</CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={metrics.filter(m => m.resting_heart_rate != null).map(m => ({ date: fmtDate(m.date), rhr: Math.round(m.resting_heart_rate!) }))}>
                  <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                  <XAxis dataKey="date" tick={{ fontSize: 10 }} className="fill-muted-foreground" interval="preserveStartEnd" />
                  <YAxis domain={["auto", "auto"]} tick={{ fontSize: 11 }} className="fill-muted-foreground" />
                  <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} labelStyle={{ color: "hsl(var(--foreground))" }} formatter={(v: number) => [`${v} bpm`, "RHR"]} />
        ...
        {metrics.some(m => m.hrv != null) && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2"><Activity className="w-4 h-4 text-primary" />Heart Rate Variability (HRV)</CardTitle>
              <CardDescription>Higher HRV indicates better recovery</CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={metrics.filter(m => m.hrv != null).map(m => ({ date: fmtDate(m.date), hrv: Math.round(m.hrv!) }))}>
                  <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                  <XAxis dataKey="date" tick={{ fontSize: 10 }} className="fill-muted-foreground" interval="preserveStartEnd" />
                  <YAxis domain={["auto", "auto"]} tick={{ fontSize: 11 }} className="fill-muted-foreground" />
                  <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} labelStyle={{ color: "hsl(var(--foreground))" }} formatter={(v: number) => [`${v} ms`, "HRV"]} />
      ...
      {metrics.some(m => m.weight != null) && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2"><WeightIcon className="w-4 h-4 text-muted-foreground" />Weight</CardTitle>
            <CardDescription>Body weight trend</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={metrics.filter(m => m.weight != null).map(m => ({ date: fmtDate(m.date), weight: Number(m.weight!.toFixed(1)) }))}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                <XAxis dataKey="date" tick={{ fontSize: 10 }} className="fill-muted-foreground" interval="preserveStartEnd" />
                <YAxis domain={["auto", "auto"]} tick={{ fontSize: 11 }} className="fill-muted-foreground" />
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} labelStyle={{ color: "hsl(var(--foreground))" }} formatter={(v: number) => [`${v} kg`, "Weight"]} />
